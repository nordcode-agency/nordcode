...
export const generateStyleString = (store: ConfigStore): string => {
    return `
        ...
        --tracking-base: ${store.trackingStandard === 0 ? 'normal' : `${store.trackingStandard}em`};
        --tracking-tight: ${store.trackingTight}em;
        --tracking-wide: ${store.trackingWide}em;

        --measure-large: ${store.measureLarge}em;
        --measure-base: ${store.measureBase}em;
        --measure-small: ${store.measureSmall}em;
