...
let items = $derived(
    Array.from(ulElement?.children ?? []).map((item) => {
        return {
            id: item.id,
...
dialog {
    display: grid;
    grid-template:
        [gallery-start controls-start] auto [controls-end] 1fr [gallery-end] / [controls-start
    gallery-start] 1fr [controls-end gallery-end];
    padding: 0;
    aspect-ratio: 9/16;
