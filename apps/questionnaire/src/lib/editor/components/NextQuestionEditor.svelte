...
let enableCustomOrder = $state(false);
...
{#each options as option}
    <div class="nc-stack">
        <div class="nc-stack -near">
            <Select label="Wenn {option.title} ausgewählt, dann gehe zu" options={allQuestionsOptions} oninput={(evt) => updateNextQuestionConfig(option.value, (event?.target as HTMLInputElement).value)} optional></Select>
        </div>
    </div>
{/each}
{/if}
